...
void MeshManager::AddMesh(Mesh* mesh)
{
	...
	for (int i = 0; i < mVertexCount; i += 3)
	{
		...
		vertices[i].Color = XMFLOAT4(vList[f.VnIndex[2]].x, vList[f.VnIndex[2]].y, vList[f.VnIndex[2]].z, 1.0f);

		vertices[i + 1].Position = vList[f.VIndex[1]];
		vertices[i + 1].Color = XMFLOAT4(vList[f.VnIndex[1]].x, vList[f.VnIndex[1]].y, vList[f.VnIndex[1]].z, 1.0f);

		vertices[i + 2].Position = vList[f.VIndex[0]];
		vertices[i + 2].Color = XMFLOAT4(vList[f.VnIndex[0]].x, vList[f.VnIndex[0]].y, vList[f.VnIndex[0]].z, 1.0f);
	}
